// Write Article Form v3 interactions (scoped .wa2-*)
(function(){
  const root = document.querySelector('.wa2-main');
  if(!root) return;
  const form = root.querySelector('#wa2-form');
  ...
  function renderCard(file){
    ...
    name.className = 'wa2-card__name';
    name.textContent = file.name;
    const size = document.createElement('span');
    size.className = 'wa2-card__size';
    ...
    if(raw){
      setTimeout(()=>{
        if(confirm('이 문서의 임시 저장본을 불러올까요?')){
          ...
          if(d.content) c.value = d.content;
